  ...
  const fetchData = async () => {
    try {
      ...
      const stationRes = await axios.get(
        `http://localhost:8080/api/station/stations/${stationId}`
      );
      const stationData = stationRes.data.stationDto;
      setStationDetails(stationData);

      const response = await axios.get(
        `http://localhost:8080/api/fuelAllocation/${stationData.stationId}/transitions`
      );
      ...
      setError("Failed to fetch data for this station.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="container pl-40 pt-24 text-white">
      ...
      >
        ...
        <div className="bg-gray-900 p-6 rounded-2xl shadow-md flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-semibold">Total Fuel Distributed</h2>
